#!/usr/bin/env python3
"""Validate and package the Agent Action Safety Chain ChatGPT skill."""
...
SKILL_NAME = "agent-action-safety-chain"
...
def validate_skill() -> list[Path]:
    ...
    required = {
        ...
        SKILL_DIR / "references" / "THREAT_CATALOG.md",
        SKILL_DIR / "references" / "COMPLETION_CHECKLIST.md",
        SKILL_DIR / "assets" / "action-proposal.schema.json",
...
def build(files: list[Path]) -> str:
    ...
    with zipfile.ZipFile(ARCHIVE, "r") as archive:
        ...
        if any(not name.startswith(expected_prefix) for name in archive.namelist()):
...
def main() -> int:
    ...
    digest = build(files)
    print(f"PASS: validated {SKILL_NAME}")
