...
CASE_FRAGMENT="$BUNDLE/asb-03-submission-case.json"
JSON_REPORT="$BUNDLE/cml-asb03-results.json"
MARKDOWN_REPORT="$BUNDLE/cml-asb03-results.md"
VERIFIER_REPORT="$BUNDLE/cml-verifier-provenance.json"

for required in "$CML_RUNNER" "$CML_BENCHMARK"; do
  if [[ ! -f "$required" ]]; then
    echo "[cml-asb-03] required verifier file not found: $required" >&2
    exit 2
  fi
done

if ! CML_COMMIT="$(git -C "$CML_ROOT" rev-parse HEAD 2>/dev/null)"; then
  echo "[cml-asb-03] CML_ROOT must be a Git checkout with a verifiable commit" >&2
  exit 2
fi
if [[ ! "$CML_COMMIT" =~ ^[0-9a-f]{40}$ ]]; then
  echo "[cml-asb-03] invalid CML commit: $CML_COMMIT" >&2
  exit 2
fi
if [[ -n "$CML_EXPECTED_COMMIT" && "$CML_COMMIT" != "$CML_EXPECTED_COMMIT" ]]; then
  echo "[cml-asb-03] verifier commit mismatch: expected $CML_EXPECTED_COMMIT, got $CML_COMMIT" >&2
  exit 2
fi
...
python3 "$CML_RUNNER" \
  ...
  --case ASB-03 \
  --agent proofpath-business-invariant-guard \
...
for key, expected in expected_summary.items():
    ...
    if actual != expected:
        raise SystemExit(
            f"[cml-asb-03] unexpected summary {key}: expected {expected!r}, got {actual!r}"
        )
if len(cases) != 1:
    raise SystemExit(f"[cml-asb-03] expected one case result, got {len(cases)}")
case = cases[0]
for key, expected in {
    "case_id": "ASB-03",
    "raw_score": 100,
    ...
    if actual != expected:
        raise SystemExit(
            f"[cml-asb-03] unexpected case {key}: expected {expected!r}, got {actual!r}"
        )
print("[cml-asb-03] independent CML score verified: ASB-03 PASS 100/100")
PY
...
provenance = {
    ...
    "invocation": {
        "case_id": "ASB-03",
        "agent": "proofpath-business-invariant-guard",
        "submission": "asb-03-submission-case.json",
    },
    "result": {
        "passed": case["passed"],
...
(
  cd "$BUNDLE"
  sha256sum \
    cml-asb03-results.json \
    cml-asb03-results.md \
    cml-verifier-provenance.json >> SHA256SUMS
  sha256sum --check SHA256SUMS
)

echo "[cml-asb-03] verifier commit: $CML_COMMIT"
echo "[cml-asb-03] reports added to $BUNDLE/"
